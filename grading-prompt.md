...
構造分解:
  - [{{r}}] {{t}}（{{n}}）                ← parts[] を再帰的に展開（下記参照）
解答: {{attempt または （未入力）}}
```

### 構造分解（parts）の展開ルール

`parts` 配列を次の形式でネスト付きリストにします（`src/utils/parts.js` の `formatPartsForCheck` と同じ）。

- トップレベルは `構造分解:` 見出しのあと、インデント2スペースから開始
- 各行: `- [役割] 英文チャンク（役割メモ）`
- `inner` がある場合は子要素をさらにインデントして追記
- 役割コード: **X**（名詞役）/ **V**（動詞）/ **Y**（形容詞役）/ **Z**（副詞役）
...
構造分解:
  - [X] The paintings（主語）
  - [V] were painted（受動態・過去形）
  - [Z] three years ago（時の副詞句）
  - [Y] displayed in this art gallery（過去分詞による後置修飾）
```
...
糸: 糸1
構造分解:
  - [V] Were ... painted（受動態の疑問文）
  - [X] the paintings（主語）
  - [Y] displayed in this art gallery（過去分詞による後置修飾）
  - [Z] by the famous artist（動作主）
  - [Z] three years ago（時の副詞句）
...
採点プロンプトには明示されていませんが、問題生成時と同じ X/Y/Z/V の意味で `parts` が付与されています。
...
feedback ではこの分解と `模範解答のポイント（nuance）` を参照して、模範解答の妥当性を説明します。

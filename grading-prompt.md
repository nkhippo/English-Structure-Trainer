---
id: pj-2026-06-15-b9da
aliases:
- pj-2026-06-15-b9da
title: 答え合わせ（採点）プロンプト仕様
created: '2026-06-15'
---
# 答え合わせ（採点）プロンプト仕様
...
上記 **疑問文採点の補足** ブロックのみ User プロンプト末尾に追加します。
...
Step 4・5・6 には Step 3/7 専用の追記はなく、**疑問文採点の補足** のみ追加します。
...
採点プロンプトには **圧縮 parts**（`t` + `r`、トップレベル＋1段 inner）を送付し、`errorTags` 付与の精度向上に使います。問題生成時に付与された parts は **模範解答（語順のまま色分け）** の表示にも使われます。

...
[例1]
  日本語: その報告書はすでに書かれている。
  英語: The report has already been written.
...
疑問文と網羅は別々に積まず、重ねる:
- **ちょうど 3 問**を「当STEPの中心構造を内包した疑問文」にする。各疑問文でも動詞情報（時制・相・態・助動詞）が学習ポイントになるよう設計。
- 網羅カテゴリは「7問全体で満たすセットの性質」であり、疑問・平叙のどちらが担ってもよい。
- 「網羅のための平叙」と「疑問」を二重に用意しない（スロットの奪い合いを起こさない）。
- **effectiveTarget（3問）を超える疑問文を作らない。** 超過しそうなら超過分は平叙文に戻す。残り 4 問は必ず平叙文（mood=declarative）

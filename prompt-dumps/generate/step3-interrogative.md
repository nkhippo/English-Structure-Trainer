---
id: pj-2026-06-23-b189
aliases:
- pj-2026-06-23-b189
title: Step 3 — interrogative（問題生成）
created: '2026-06-23'
---
# Step 3 — interrogative（問題生成）
...
[例2]
  日本語: 彼は昨日、その問題を解決した。
  英語: He solved the problem yesterday.
...
3. 各文が自然な日本語かつ当STEPの中心構造（Step3=動詞情報（時制・相・態・助動詞））を内包した疑問文であること
4. questionType の多様性（同型連発禁止）
5. テーマ多様性
※ 本セッションは疑問 production 特化。構造網羅（4カテゴリ・Y+Z 等）は要求しない

疑問ドリル（production）:
- 設計思想: 当STEPの中心構造（動詞情報（時制・相・態・助動詞））を内包したまま疑問変形（法=mood）をかぶせる
...
Step 3 疑問文の設計（必須）:
- 5問すべて mood=interrogative。**yesno と wh の両方**を含める
...
スコープの絶対条件:
- 全5問が「動詞情報（時制・相・態・助動詞）」を主たる練習点にすること。
- 他STEPの構造（関係詞節・節ネスト・発展操作）が文の主目的になっている文を混ぜない。
- × Step3で動名詞句が主役の問を増やしすぎない（Step4寄り）
...
- 5問で扱う文法パターン（動詞の活用（時制・相・態）と助動詞 ＋ Yes/No疑問・wh疑問・not・短縮形）もできるだけバラけさせ、似た構文の連続を避ける

...
- effectiveTarget（優先順位3）: 7
- maxNatural: 7 — （STEP上限 7問 = effectiveTarget 7）
...
[例1]
  日本語: あなたは昨日、その本を買いましたか。
  英語: Did you buy the book yesterday?
...
3. 当STEP範囲の疑問文を**ちょうど 7 問**にする（**超過も不足もしない**。不足時のみ減らして _questionNote）。残り 0 問は平叙文（mood=declarative）
...
疑問文と網羅は別々に積まず、重ねる:
- **ちょうど 7 問**を「当STEPの中心構造を内包した疑問文」にする。各疑問文でも動詞情報（時制・相・態・助動詞）が学習ポイントになるよう設計。
- 網羅カテゴリは「7問全体で満たすセットの性質」であり、疑問・平叙のどちらが担ってもよい。
- 「網羅のための平叙」と「疑問」を二重に用意しない（スロットの奪い合いを起こさない）。
- **effectiveTarget（7問）を超える疑問文を作らない。** 超過しそうなら超過分は平叙文に戻す。残り 0 問は必ず平叙文（mood=declarative）
...
- 7問のうち **yesno と wh の両方**を含める（preferred: mix）
...
5. 返却直前に mood=interrogative の件数を数え、**effectiveTarget=7 と一致**するか確認する（多すぎる場合は超過分を平叙に戻してから返す。不足時のみ _questionNote）
...
- **法(mood)軸は構造軸を侵食しないこと**。同型の疑問文（例: Step5『[名詞＋関係詞節]は〜ですか』）を effectiveTarget（7問）を超えて並べない。構造（関係詞サブタイプ等）のばらつきを優先する

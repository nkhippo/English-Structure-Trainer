...
[例1]
  日本語: 彼女にはまだ読んでいない本がたくさんある。
  英語: She has many books to read.

[例2]
  日本語: その問題はとても難しい。
  英語: The problem is very difficult.
...
2. 各文が母語話者に自然な日本語であり、かつ当STEPの中心構造（Step4=役割(X/Y/Z)と準動詞/前置詞句）が主たる練習点であること
3. 当STEP範囲の疑問文を**ちょうど 5 問**にする（**超過も不足もしない**。不足時のみ減らして _questionNote）。残り 2 問は平叙文（mood=declarative）
4. 役割≠形の minimal pair・同形異役割 をセット全体で網羅（疑問・平叙のどちらが担ってもよい）
5. テーマ多様性
...
疑問文と網羅は別々に積まず、重ねる:
- **ちょうど 5 問**を「当STEPの中心構造を内包した疑問文」にする。疑問文の骨格内に to do / -ing / 前置詞句を載せる。
...
スコープの絶対条件:
- 全7問が「役割(X/Y/Z)と準動詞/前置詞句」を主たる練習点にすること。

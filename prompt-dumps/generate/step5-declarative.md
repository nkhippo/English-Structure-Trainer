---
id: pj-2026-06-23-21e4
aliases:
- pj-2026-06-23-21e4
title: Step 5 — declarative（問題生成）
created: '2026-06-23'
---
# Step 5 — declarative（問題生成）

- 仕様: work-request-mode-split 改修5（平叙/疑問モード分離・mixed廃止）
- 生成元: `buildGeneratePrompt()` in `src/prompts/index.js`
- generationMode: declarative
- 問数 n: 7
...
4. 関係代名詞・関係副詞・what名詞節・同格that をセット全体で網羅
必達（Step5）: 7問のうち**最低1問**は「関係詞節(gap有) vs 同格節(gap無)」を対比し、Step5エッセンス（gapの有無）を体現する
5. テーマ多様性

平叙セット（構造網羅）:
- 設計思想: 1セット7問すべて平叙文で、当STEPの MECE 網羅（STEP_COVERAGE）を担う

...
3. 各文が自然な日本語かつ当STEPの中心構造（Step5=関係詞節（後置修飾））を内包した疑問文であること
4. questionType の多様性（同型連発禁止）
5. テーマ多様性
※ 本セッションは疑問 production 特化。構造網羅（4カテゴリ・Y+Z 等）は要求しない

疑問ドリル（production）:
- 設計思想: 当STEPの中心構造（関係詞節（後置修飾））を内包したまま疑問変形（法=mood）をかぶせる
...
スコープの絶対条件:
- 全5問が「関係詞節（後置修飾）」を主たる練習点にすること。
- 他STEPの構造（分詞・副詞節・準動詞（to do/-ing）が主目的の文）が文の主目的になっている文を混ぜない。
- × Step5に副詞節(when/because…が主役)・分裂文(it is…that)を主練習点にしない
...
- 5問で扱う文法パターン（関係代名詞（who/which/that）・関係副詞（where/when）・whatの名詞節）もできるだけバラけさせ、似た構文の連続を避ける

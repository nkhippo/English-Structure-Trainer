...
- effectiveTarget（優先順位3）: 3
- maxNatural: 3 — スライダー 7 → effectiveTarget 3
...
文法ポイント: 関係詞（関係代名詞（who/which/that）・関係副詞（where/when）・whatの名詞節）

このSTEPの再利用原理（エッセンス）— nuance 末尾でどれを使ったか1行で示すこと:
後置修飾＝核を先に・説明を後ろに（head-initial）。**関係詞節＝空所(gap)あり／同格節＝完成形(gapなし)**。what は先行詞内蔵。

参考例（日本語の自然さ・文体の基準。テーマや内容は参考例に引きずられず、下記のテーマ割り当てに従うこと）:
[例1]
  日本語: 彼女には読むべき（仕事上推奨される）本がたくさんある。
  英語: She has many books she should read.

[例2]
  日本語: これが私が住んでいる街だ。
  英語: This is the city where I live.
...
2. 各文が母語話者に自然な日本語であり、かつ当STEPの中心構造（Step5=関係詞節（後置修飾））が主たる練習点であること
3. 当STEP範囲の疑問文を**ちょうど 3 問**にする（**超過も不足もしない**。不足時のみ減らして _questionNote）。残り 4 問は平叙文（mood=declarative）
4. 関係代名詞・関係副詞・what名詞節・同格that をセット全体で網羅（疑問・平叙のどちらが担ってもよい）
必達（Step5）: 7問のうち**最低1問**は「関係詞節(gap有) vs 同格節(gap無)」を対比し、Step5エッセンス（gapの有無）を体現する（平叙文で可）
5. テーマ多様性
...
- スライダー目標: 7問（スライダー 7問 → maxNatural により目標 3問）
- STEP疑問ポリシー（タイプはポリシーで自動選択。allowed 外は生成禁止）:
- 許可タイプ（allowedTypes）: Yes/No疑問（糸1：助動詞前置） — **allowed 外のタイプは生成しない**
- 優先（preferred）: Yes/No疑問（糸1：助動詞前置） を優先
- 何を疑問にするか: 関係詞節(Y)を内包したまま主骨格を疑問化（例：その車を直した男はまだここにいますか）
- 自然な上限（maxNatural）: 3問

疑問文と網羅は別々に積まず、重ねる:
- **ちょうど 3 問**を「当STEPの中心構造を内包した疑問文」にする。各疑問文の Y に関係代名詞節または関係副詞節を載せる（what名詞節・同格that は平叙側でカバー）。
- 網羅カテゴリは「7問全体で満たすセットの性質」であり、疑問・平叙のどちらが担ってもよい。
- 「網羅のための平叙」と「疑問」を二重に用意しない（スロットの奪い合いを起こさない）。
- 関係詞4カテゴリの網羅と「関係詞節 vs 同格節」の対比(1問)は、**残りの平叙文（4問）で必ず満たす**。疑問文は網羅に寄与してよいが、**網羅の達成責任は平叙側に置く**（疑問はYes/Noコピュラに偏り同格that・gap対比を締め出すため）
- **effectiveTarget（3問）を超える疑問文を作らない。** 超過しそうなら超過分は平叙文に戻す。残りスロットは必ず mood=declarative
...
Step 5 疑問文の設計（Yes/No のみ — 上限 3 問）:
...
- **同型の『[名詞＋関係詞節]は〜ですか』を effectiveTarget（3問）を超えて並べない**
...
5. 返却直前に mood=interrogative の件数を数え、**effectiveTarget=3 と一致**するか確認する（多すぎる場合は超過分を平叙に戻してから返す。不足時のみ _questionNote）
...
スコープの絶対条件:
- 全7問が「関係詞節（後置修飾）」を主たる練習点にすること。
- 他STEPの構造（分詞・副詞節・準動詞（to do/-ing）が主目的の文）が文の主目的になっている文を混ぜない。
- **法(mood)軸は構造軸を侵食しないこと**。同型の疑問文（例: Step5『[名詞＋関係詞節]は〜ですか』）を effectiveTarget（3問）を超えて並べない。構造（関係詞サブタイプ等）のばらつきを優先する
...
- en は「意味が通る訳」ではなく、**Step の文法ポイント（関係代名詞（who/which/that）・関係副詞（where/when）・whatの名詞節）を最も明確に示す**模範訳とする
...
- 7問で扱う文法パターン（関係代名詞（who/which/that）・関係副詞（where/when）・whatの名詞節）もできるだけバラけさせ、似た構文の連続を避ける
- JSON配列の並び順は問ごとにランダムにする（テーマ割り当ての順番と一致させない）
Step 5 固有の出題制約（必須）:
- 関係代名詞・関係副詞・what名詞節・同格that を**別カテゴリとして**カバー
- 関係詞節 vs 同格節の対比を1問

...
- effectiveTarget（優先順位3）: 5
- maxNatural: 5 — スライダー 7 → effectiveTarget 5
...
文法ポイント: 節と接続・ネスト（副詞節（when/because/if/although）・that名詞節・等位接続・関係詞と副詞節のネスト）

このSTEPの再利用原理（エッセンス）— nuance 末尾でどれを使ったか1行で示すこと:
節も小さな文（内部に X+V）→再帰。等位(対等) vs 従属(主従)。従属節の役割は X(名詞節)/Y(関係詞節)/Z(副詞節) でMECE。

参考例（日本語の自然さ・文体の基準。テーマや内容は参考例に引きずられず、下記のテーマ割り当てに従うこと）:
[例1]
  日本語: 彼女が疲れているとき、彼女は早く帰る。
  英語: When she is tired, she goes home early.

[例2]
  日本語: 私は彼が正直だと知っている。
  英語: I know that he is honest.
...
2. 各文が母語話者に自然な日本語であり、かつ当STEPの中心構造（Step6=節のネスト（等位/従属・名詞節/関係詞節/副詞節））が主たる練習点であること
3. 当STEP範囲の疑問文を**ちょうど 5 問**にする（**超過も不足もしない**。不足時のみ減らして _questionNote）。残り 2 問は平叙文（mood=declarative）
4. Y+Zキーセンテンス型・副詞節(Z)・名詞節(X)・等位接続 をセット全体で網羅（疑問・平叙のどちらが担ってもよい）
5. テーマ多様性
...
- スライダー目標: 7問（スライダー 7問 → maxNatural により目標 5問）
- STEP疑問ポリシー（タイプはポリシーで自動選択。allowed 外は生成禁止）:
- 許可タイプ（allowedTypes）: Yes/No疑問（糸1：助動詞前置） / wh疑問（糸2：空所を文頭へ） / 間接疑問（wh/if 節を名詞節Xとして埋め込む） — **allowed 外のタイプは生成しない**
- 優先（preferred）: 間接疑問（wh/if 節を名詞節Xとして埋め込む） を優先
- 何を疑問にするか: wh/if 節を名詞節Xとして埋め込む。直接疑問との対比も可
- 自然な上限（maxNatural）: 5問
- 備考: 主語wh疑問（"Who broke it?"）は助動詞前置も移動も起きず平叙文に見える——時々混ぜ、間接疑問との対比で糸1/糸2の実感を持てるようにする

疑問文と網羅は別々に積まず、重ねる:
- **ちょうど 5 問**を「当STEPの中心構造を内包した疑問文」にする。間接疑問の wh/if 名詞節(X) や Y+Z キーセンテンス型を疑問文に載せる。
- 網羅カテゴリは「7問全体で満たすセットの性質」であり、疑問・平叙のどちらが担ってもよい。
- 「網羅のための平叙」と「疑問」を二重に用意しない（スロットの奪い合いを起こさない）。
- **effectiveTarget（5問）を超える疑問文を作らない。** 超過しそうなら超過分は平叙文に戻す。残り 2 問は必ず平叙文（mood=declarative）
...
- 5問のうち **過半数は indirect**（wh/if 節を名詞節Xとして埋め込む）。残りは直接 yesno/wh
...
5. 返却直前に mood=interrogative の件数を数え、**effectiveTarget=5 と一致**するか確認する（多すぎる場合は超過分を平叙に戻してから返す。不足時のみ _questionNote）
...
スコープの絶対条件:
- 全7問が「節のネスト（等位/従属・名詞節/関係詞節/副詞節）」を主たる練習点にすること。
- 他STEPの構造（関係詞単体練習・発展操作が主目的の文）が文の主目的になっている文を混ぜない。
- **法(mood)軸は構造軸を侵食しないこと**。同型の疑問文（例: Step5『[名詞＋関係詞節]は〜ですか』）を effectiveTarget（5問）を超えて並べない。構造（関係詞サブタイプ等）のばらつきを優先する
...
- en は「意味が通る訳」ではなく、**Step の文法ポイント（副詞節（when/because/if/although）・that名詞節・等位接続・関係詞と副詞節のネスト）を最も明確に示す**模範訳とする
...
- 7問で扱う文法パターン（副詞節（when/because/if/although）・that名詞節・等位接続・関係詞と副詞節のネスト）もできるだけバラけさせ、似た構文の連続を避ける
- JSON配列の並び順は問ごとにランダムにする（テーマ割り当ての順番と一致させない）
Step 6 固有の出題制約（必須）:
- **キーセンテンス型（Y+Z共起：People who… work hard when…）を最低1問**＝課題③本丸

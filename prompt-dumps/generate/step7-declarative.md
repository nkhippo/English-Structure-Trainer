...
[例2]
  日本語: 会議に出席したのはすべての人ではない。
  英語: Not all of them attended the meeting.
...
4. operationTag 2〜3種混在・倒置必須 をセット全体で網羅
5. テーマ多様性
...
STEP_COVERAGE（必須）:
- 既存の operationTag サンプリング（直前セットと同じタグ構成を避ける）を維持
...
- × Step7で裸の不定詞主役・基本wh疑問(What did he…)のみで終わらせない
...
Step 7 固有の出題制約（必須）:
- 各問に operationTag を1つ付与（比較 / 仮定法 / 倒置/強調 / 否定 / 話法 / 省略 のいずれか）

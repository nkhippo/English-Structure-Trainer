...
- effectiveTarget（必達）: 7
- maxNatural: スライダー7 → 必達7
...
疑問文必達: 返却JSON配列に mood=interrogative の要素を effectiveTarget=7 件以上含めること（0件は不可）。
...
- **必達疑問文数（effectiveTarget）**: 7問 / 7問 — **mood=interrogative がこの件数以上含まれること（0件不可）**
...
- **生成順序（必須）**:
  1. 先に 7 問分の疑問文スロットを確定（mood=interrogative、questionType、thread）
  2. 残り 0 問を平叙文（mood=declarative または省略）で設計
  3. 返却直前に mood=interrogative の件数を数え、7 未満なら疑問文を追加してから返す
...
生成手順（必ずこの順番で）:
0. **疑問文 7 問を先に割り当て**（各問 mood=interrogative / questionType / thread を決める — yesno と wh を混在。動詞情報（時制・相・態・助動詞）を問う）

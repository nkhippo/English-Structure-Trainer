...
[例1]
  日本語: 音楽を聴きながら勉強した彼女は、試験に合格した。
  英語: She studied listening to music and passed the exam.
...
3. 当STEP範囲の疑問文を 2 問含める（疑問文は当STEPの構造を内包し、網羅を兼ねる）
...
疑問文と網羅は別々に積まず、重ねる:
- 2 問を「当STEPの中心構造を内包した疑問文」にする。間接疑問の wh/if 名詞節(X) や Y+Z キーセンテンス型を疑問文に載せる。
...
- 2問のうち **過半数は indirect**（wh/if 節を名詞節Xとして埋め込む）。残りは直接 yesno/wh

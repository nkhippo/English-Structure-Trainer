...
疑問文必達: 返却JSON配列に mood=interrogative の要素を effectiveTarget=2 件以上含めること（0件は不可）。
...
- **必達疑問文数（effectiveTarget）**: 2問 / 7問 — **mood=interrogative がこの件数以上含まれること（0件不可）**
- スライダー目標（requested）: 2問
...
- **生成順序（必須）**:
  1. 先に 2 問分の疑問文スロットを確定（mood=interrogative、questionType、thread）
  2. 残り 5 問を平叙文（mood=declarative または省略）で設計
  3. 返却直前に mood=interrogative の件数を数え、2 未満なら疑問文を追加してから返す
...
生成手順（必ずこの順番で）:
0. **疑問文 2 問を先に割り当て**（各問 mood=interrogative / questionType / thread を決める — 間接疑問を中心に（過半数 indirect））

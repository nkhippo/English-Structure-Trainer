...
文法ポイント: 発展構文（骨格への7操作（比較・仮定法・疑問の体系・倒置/強調・否定・話法・省略））

このSTEPの再利用原理（エッセンス）— nuance 末尾でどれを使ったか1行で示すこと:
糸1=糸1（助動詞前置）／糸2=糸2（空所＋移動）。**糸は疑問・倒置・強調・仮定法倒置を束ねるが、比較・省略・話法はどちらにも乗らない**＝糸は部分的統一原理であって網羅ではない。
...
- スライダー 7問は maxNatural により effectiveTarget=2問が必達上限
...
- 何を疑問にするか: 糸の二重適用を回避（倒置=糸1のときは wh=糸2、強調cleft=糸2のときは Yes/No=糸1）。operationTag「疑問」の問はこの方針に従う
- 自然な上限（maxNatural）: 2問
- **生成順序（必須）**:
...
生成手順（必ずこの順番で）:
0. **疑問文 2 問を先に割り当て**（各問 mood=interrogative / questionType / thread を決める — operationTag「疑問」を effectiveTarget 件確保）
...
- 7問で扱う文法パターン（骨格への7操作（比較・仮定法・疑問の体系・倒置/強調・否定・話法・省略））もできるだけバラけさせ、似た構文の連続を避ける
- JSON配列の並び順は問ごとにランダムにする（テーマ割り当ての順番と一致させない）
Step 7 固有の出題制約（必須）:
- 各問に operationTag を1つ付与（比較 / 仮定法 / 疑問 / 倒置/強調 / 否定 / 話法 / 省略 のいずれか）
- 1セット（7問）で **2〜3種類の operationTag を混在** させる。同じタグが3問以上連続しないよう並べを工夫する
- （直前セットなし — 2〜3種類の操作タグをバランスよく混在させる）
- **最低1問** は operationTag「倒置/強調」かつ、否定副詞句（no longer / no more / never again / hardly ever / not only / by no means / in no way / under no circumstances / on no account / seldom / rarely / little 等）による倒置を含める

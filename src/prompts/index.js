...
import { buildDeclarativeSection } from './questionPractice/declarative.js';
import { buildInterrogativeOnlySection } from './questionPractice/interrogativeOnly.js';
import { buildNaturalnessAbsoluteSection, STEP_CENTER_STRUCTURE } from './questionPractice/shared.js';
...
function buildStep3GenerateExtra(n) {
  return `
Step 3 固有の出題制約（必須）:
- 全問 mood=declarative（平叙文）。疑問文は含めない
- 否定文を1問以上含めてもよい（mood=declarative のまま）
- 時制・相・態・助動詞の問題をバランスよく含める
...
返却形式への追加フィールド（Step 7 のみ必須）:
  "operationTag": "比較|仮定法|倒置/強調|否定|話法|省略",
  "cefr": "A2|B1|B2|C1",
  "thread": "糸1|糸2"`;
}
...
- 操作タグ（比較・仮定法・倒置/強調・否定・話法・省略）に関わらず、意味と文法が正しければ満点に近い評価とする
...
function buildStepGenerateExtra(step, n, generationMode) {
  if (generationMode === 'interrogative') return '';
...
function buildMoodJsonSchema(generationMode) {
  ...
  return `
    "mood": "declarative（全問必須）"`;
}

export function buildGeneratePrompt(stepInfo, n, { step, reviewMarkdown, coreTagSummary, generationMode: rawMode } = {}) {
  ...
  const stepExtra = buildStepGenerateExtra(step, n, generationMode);
  const modeSection = !reviewMarkdown && step >= 3 && step <= 7
    ? (generationMode === 'interrogative'
      ? buildInterrogativeOnlySection(step, n)
      : buildDeclarativeSection(step, n))
    : '';
  const naturalnessSection = generationMode === 'declarative' && step >= 3 && step <= 7 && !modeSection.includes('自然さの絶対条件')
    ? buildNaturalnessAbsoluteSection(step, n)
    : '';
  const moodCheckStep = generationMode === 'interrogative'
    ? `\n5. 返却直前に全問 mood=interrogative であることを確認する`
    : generationMode === 'declarative' && step >= 3
      ? `\n5. 返却直前に全問 mood=declarative であることを確認する（疑問文が1問もないこと）`
      : '';
  ...
  const followUpSection = reviewMarkdown
    ...
    : '';
  const enReplySection = generationMode === 'interrogative' ? `
...
[
  {
    ...
    ],${buildMoodJsonSchema(generationMode)}
  }
]
...
const SINGLE_REGEN_ISSUE_NOTES = {
  ...
  wrong_mood: 'mood がモード方針に違反している。平叙モードでは declarative、疑問ドリルでは interrogative に統一する。',
  unnatural: '日本語が未完・ハイブリッド・過剰技巧。完結した自然な1文に書き直す。',
  missing_yz_key: 'Y+Zキーセンテンス型（People who… work hard when…）を主練習点にした1問を生成する。',
  step5_diversity: 'Step5 の関係詞型が単調。別の関係詞サブタイプ（主格/目的格/関係副詞/what名詞節/同格that）で書き直す。',
};
...
export function buildSingleExerciseRegeneratePrompt(stepInfo, step, n, {
  issue, index, siblingJps = [], mood = 'declarative', generationMode = 'declarative',
} = {}) {
  const policyBlock = generationMode === 'interrogative' ? formatQuestionPolicyForPrompt(step) : '';
...
${siblingJps.length ? siblingJps.map((jp) => `- ${jp}`).join('\n') : '（なし）'}

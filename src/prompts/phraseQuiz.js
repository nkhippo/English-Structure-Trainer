...
日本語の題材文（jp）は、誤答を誘発するあいまいさを避け、正解フレーズに一意に対応するよう書いてください。
...
[
  {
    "expr": "指定されたフレーズ（そのまま）",
    "jp": "正解に一意に対応する日本語文（冗長でもよい。多義解釈を与えない）",
...
問題文（jp / en）のルール — jp は最重要:
- jp は **多義解釈を与えない**こと。1〜2文でよいが、必要なら冗長に書いてよい
- 日本語だけを読んだとき、**正解 expr だけ**が自然に浮かぶようにすること（distractors が jp からも正解に見える書き方は禁止）
- expr の note・category が示すニュアンス（フォーマル度、話題の切り出し、追加・逆接・譲歩・否定の強さなど）を jp に必ず織り込むこと
- 悪い例: 「予定の遅延は天候の悪さのせいです。」→ because of / due to のどちらもありそう（フォーマル度が不明）
- 良い例: 「工程の遅れについて、社外向けの公式報告では悪天候が原因とされています。」→ 公式・ビジネス文脈が伝わり due to が自然
- 悪い例: 「田中さんは来られません。山田さんは？」→ as for / about の区別が弱い
- 良い例: 「出席について、田中さんは来られない見込みです。さて、山田さんの方はいかがでしょうか。」→ 話題の切り替えが明確で as for が自然
- en と jp の対応は正解 expr で成立すること。en は jp の意味を忠実に反映すること

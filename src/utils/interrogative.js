...
export function inferInterrogativeMood(ex) {
  ...
  if (/^(Do|Does|Did|Is|Are|Am|Was|Were|Have|Has|Had|Will|Would|Can|Could|Should|May|Might|What|Who|Where|When|Why|How|Which)\b/i.test(en)) {
  ...
  if (/(かどうか|知りたい|わからない|教えて|尋ね|聞きたい|確認したい|不明|何を|誰が|どこ|いつ|なぜ|どう)/.test(jp)) {
    return 'interrogative';
  }
